...
The tree is owned by `bt_node` (a ROS 2 node) and ticked at **10 Hz**. State flows in from topics in the `/kumi` namespace; the tree drives the trajectory publisher that talks to Isaac Sim.
...
│   └── ExecuteWalk                   enables the trajectory publisher
│
...
All topics are under the `/kumi` namespace.

State subscriptions:

| Topic | Type | Source |
|---|---|---|
| `/kumi/kumi_behavior/emergency` | `std_msgs/Bool` | Control GUI |
| `/kumi/kumi_seq_traj_controller/enabled` | `std_msgs/Bool` | Control GUI |
| `/kumi/kumi_seq_traj_controller/gait` | `std_msgs/String` | Control GUI |

The tree drives the trajectory publisher through the same enable / gait topics.
...
Or include it in the full Isaac bringup via [`kumi_bringup`](../kumi_bringup/README.md).
