...
The package is reserved for the perception side of the stack. It will consume the on-robot cameras rendered by Isaac Sim (front RGB + depth, plus the optional corner RGB / depth pairs declared in [`kumi_description`](../kumi_description/README.md)) and produce higher-level state for downstream consumers — the behavior tree first, eventually a planner.
...
The package will subscribe to the camera topics published by Isaac Sim through the ROS bridge:

| Topic | Type |
|---|---|
| `/kumi/front_camera/image` | `sensor_msgs/Image` |
| `/kumi/front_camera/camera_info` | `sensor_msgs/CameraInfo` |
| `/kumi/front_depth/image` | `sensor_msgs/Image` |
| `/kumi/front_depth/camera_info` | `sensor_msgs/CameraInfo` |
